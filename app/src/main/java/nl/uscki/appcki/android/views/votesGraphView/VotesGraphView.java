...
//        drawBar(canvas);
    ...
    private final int bar_margin = (int)convertDpToPixel(1.666666f);


    /**
     * Calculate the effective content width that can be used to draw bars. This
     * is the actual content width, minus all space used for padding and rounded corners
     */
    protected int getEffectiveContentWidth(int nBars) {
        return width - (getPaddingLeft() + getPaddingRight() +
            nBars * round_bar_offset +
            bar_margin);
    }

    /**
     * Draw a default bar, starting from <i>start</i>, <i>width</i> wide. Excluding padding and
     * rounded corners, which are added by this method
     * @param start         Start position of the bar
     * @param width         Desired width of the bar
    ...
    protected void drawRectangle(int start, int width, boolean roundedLeft, Paint paint, Canvas canvas) {
        ...
        if(roundedLeft) {
            ...
            roundedCorner = new RectF(start + width - 2*round_bar_size, 0,  width, BAR_HEIGHT);
        }

        squareBar = new RectF(start, 0, start + width - round_bar_size, BAR_HEIGHT);
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        setMeasuredDimension(measure() + round_bar_size, (int) BAR_HEIGHT);
    }
